...
var SrtTranslator = class {
  constructor() {
    this.parser = new dist_default();
  ...
  async translateBatch(subtitles, batchLines, prevContext, nextContext, targetLanguage, model, movieContext, tone) {
...
2. Adjust pronouns and addressing (x\u01B0ng h\xF4) automatically based on the relationship and situation (e.g., in Vietnamese use m\xE0y/tao for fights/gangsters, t\xF4i/b\u1EA1n for normal, anh/em for couples, etc., depending on the current context).
...
Return ONLY a JSON array of strings, where each string is the translation of the corresponding line. The length of the array MUST be exactly ${batchLines.length}. DO NOT combine lines. Maintain the same formatting (e.g., italic tags <i>...</i> or newlines if present).`;
    const response = await this.ai.models.generateContent({
      ...
      config: {
        ...
        temperature: 0.3
        // Lower temperature for more accurate translation
      }
    });
  ...
  async translate(srtText, options) {
    if (options.apiKey) {
  ...
  async rewriteSubtitle(original, currentTranslation, targetLanguage, movieContext, tone, model) {
...
Return ONLY the rewritten translated text as a plain string, with no quotes or extra formatting around it.`;
  ...
  async summarizeSubtitles(subtitles, movieContext, model = "gemini-2.5-flash") {
...
Please provide a concise but comprehensive summary of the story based on these subtitles. Write the summary prioritizing the provided context language, and provide insight into the main characters and plot points. Note: Depending on the length of the script, it could be cut short, so summarize what is available.`;
  ...
  async evaluateTranslation(originalSubtitles, translatedSubtitles, targetLanguage, movieContext, model = "gemini-2.5-flash") {
